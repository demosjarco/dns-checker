...
export const output = z.record(
	...
	z.union([
		...
		z.array(z.union([z.string(), z.record(z.string(), z.any()), z.array(z.string()), z.array(z.record(z.string(), z.any()))])).openapi({
		...
		if (lockedDo) {
			...
			const responses = await Promise.all(
				servers.map((server) =>
					wrapTime(
						c,
						`Do${server.protocol === 'https:' ? 'H' : server.protocol === 'tls:' ? 'T' : '?'}-${server.hostname}`,
						doStub
							.getDnsQuery(server.toString(), c.req.valid('param').hostname, c.req.valid('param').rrtype, c.var.browserCachePolicy)
							.then((response) => ({
								[server.toString()]: response,
							}))
							.catch((error) => {
								console.error(server.toString(), error);
