CREATE TABLE `instances` (
	...
	`do_id_utf8` text GENERATED ALWAYS AS (lower(hex("do_id"))) VIRTUAL,
	`location` text NOT NULL,
	`iata` text NOT NULL,
	`colo` integer NOT NULL,
	`iso_country` text NOT NULL,
	`iso_region` text NOT NULL,
	FOREIGN KEY (`location`) REFERENCES `locations`(`location`) ON UPDATE cascade ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `instances_iata_colo_unique` ON `instances` (`iata`,`colo`);--> statement-breakpoint
